"use client";
import React, { useState } from 'react';

// Create New Playlist Modal Component
const CreateNewPlaylistModal = ({ isOpen, onClose, onPlaylistCreated }) => {
  const [formData, setFormData] = useState({
    title: '',
    artist: '',
    description: '',
    coverImage: null
  });

  const [dragOver, setDragOver] = useState(false);

  const handleInputChange = (e) => {
    const { name, value } = e.target;
    setFormData(prev => ({
      ...prev,
      [name]: value
    }));
  };

  const handleFileChange = (e) => {
    const file = e.target.files[0];
    setFormData(prev => ({
      ...prev,
      coverImage: file
    }));
  };

  const handleDragOver = (e) => {
    e.preventDefault();
    setDragOver(true);
  };

  const handleDragLeave = (e) => {
    e.preventDefault();
    setDragOver(false);
  };

  const handleDrop = (e) => {
    e.preventDefault();
    setDragOver(false);
    
    const file = e.dataTransfer.files[0];
    setFormData(prev => ({
      ...prev,
      coverImage: file
    }));
  };

  const handleCancel = () => {
    setFormData({
      title: '',
      artist: '',
      description: '',
      coverImage: null
    });
    onClose();
  };

  const handleAddSongs = () => {
    const newPlaylist = {
      id: Date.now(),
      name: formData.title || 'New Playlist',
      visibility: 'Public',
      tracks: 0,
      updatedDate: new Date().toISOString().split('T')[0],
    };
    
    onPlaylistCreated(newPlaylist);
    setFormData({
      title: '',
      artist: '',
      description: '',
      coverImage: null
    });
    onClose();
  };

  if (!isOpen) return null;

  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
      <div className="bg-[#312B36] rounded-lg w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        {/* Header */}
        <div className="flex items-center justify-between p-6 border-b border-[#896E9C]">
          <h2 className="text-xl font-medium text-[#F9FAFB]">Upload New Song</h2>
          <button 
            onClick={onClose}
            className="p-2 hover:bg-[#1D1B25] rounded-lg transition-colors"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              className="h-5 w-5 text-[#F9FAFB]"
              viewBox="0 0 20 20"
              fill="currentColor"
            >
              <path
                fillRule="evenodd"
                d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z"
                clipRule="evenodd"
              />
            </svg>
          </button>
        </div>

        {/* Content */}
        <div className="p-6">
          {/* Upload Cover */}
          <div className="mb-6">
            <label className="block text-sm font-medium text-[#F9FAFB] mb-3">
              Upload Cover
            </label>
            <div
              className={`relative border-2 border-dashed rounded-lg p-12 text-center transition-colors ${
                dragOver 
                  ? 'border-[#FF7DD0] bg-[#FF7DD0]/10' 
                  : 'border-[#896E9C] hover:border-[#A38BB4]'
              }`}
              onDragOver={handleDragOver}
              onDragLeave={handleDragLeave}
              onDrop={handleDrop}
            >
              <input
                type="file"
                accept="image/*"
                onChange={handleFileChange}
                className="absolute inset-0 w-full h-full opacity-0 cursor-pointer"
              />
              <div className="flex flex-col items-center">
                <svg
                  xmlns="http://www.w3.org/2000/svg"
                  className="h-16 w-16 text-[#896E9C] mb-4"
                  fill="none"
                  viewBox="0 0 24 24"
                  stroke="currentColor"
                >
                  <path
                    strokeLinecap="round"
                    strokeLinejoin="round"
                    strokeWidth={2}
                    d="M4 16l4.586-4.586a2 2 0 012.828 0L16 16m-2-2l1.586-1.586a2 2 0 012.828 0L20 14m-6-6h.01M6 20h12a2 2 0 002-2V6a2 2 0 00-2-2H6a2 2 0 00-2 2v12a2 2 0 002 2z"
                  />
                </svg>
                <h3 className="text-lg font-medium text-[#F9FAFB] mb-2">Upload Cover</h3>
                <p className="text-sm text-[#896E9C]">
                  {formData.coverImage 
                    ? formData.coverImage.name 
                    : 'Image must be in JPG or PNG format and at least 300x300 pixels'
                  }
                </p>
              </div>
            </div>
          </div>

          {/* Form Fields */}
          <div className="space-y-4 mb-8">
            {/* Title */}
            <div>
              <label className="block text-sm font-medium text-[#F9FAFB] mb-2">
                Title
              </label>
              <input
                type="text"
                name="title"
                value={formData.title}
                onChange={handleInputChange}
                placeholder="Enter song title"
                className="w-full px-4 py-3 bg-[#1D1B25] text-white rounded-md border border-[#896E9C] focus:outline-none focus:border-[#A38BB4] transition-colors"
              />
            </div>

            {/* Artist */}
            <div>
              <label className="block text-sm font-medium text-[#F9FAFB] mb-2">
                Artist
              </label>
              <input
                type="text"
                name="artist"
                value={formData.artist}
                onChange={handleInputChange}
                placeholder="Enter artist name"
                className="w-full px-4 py-3 bg-[#1D1B25] text-white rounded-md border border-[#896E9C] focus:outline-none focus:border-[#A38BB4] transition-colors"
              />
            </div>

            {/* Description */}
            <div>
              <label className="block text-sm font-medium text-[#F9FAFB] mb-2">
                Description
              </label>
              <textarea
                name="description"
                value={formData.description}
                onChange={handleInputChange}
                placeholder="Give your playlist a catchy description..."
                rows="4"
                className="w-full px-4 py-3 bg-[#1D1B25] text-white rounded-md border border-[#896E9C] focus:outline-none focus:border-[#A38BB4] transition-colors resize-none"
              />
            </div>
          </div>

          {/* Action Buttons */}
          <div className="flex justify-start space-x-4">
            <button
              onClick={handleCancel}
              className="px-6 py-3 bg-[#F7009E33] text-[#F9FAFB] rounded-md hover:bg-[#2A374B] transition-colors"
            >
              Cancel
            </button>
            <button
              onClick={handleAddSongs}
              className="px-6 py-3 bg-gradient-to-b from-[#FF7DD0] to-[#F7009E] hover:from-[#FF6BC9] hover:to-[#E6008F] text-white rounded-md transition-all duration-200"
            >
              Add songs
            </button>
          </div>
        </div>
      </div>
    </div>
  );
};

// Add Songs Modal Component
const AddSongsModal = ({ isOpen, onClose }) => {
  const [selectedSongs, setSelectedSongs] = useState({});
  
  const songs = [
    { id: 1, title: '2Am Vibes', time: '3:52', image: '/image/song-1.png' },
    { id: 2, title: 'Ghetto Love', time: '4:10', image: '/image/song-2.png' },
    { id: 3, title: 'Midnight Dreams', time: '2:36', image: '/image/song-3.png' },
    { id: 4, title: 'Echoes of the Past', time: '5:18', image: '/image/song-4.png' },
    { id: 5, title: 'Silent Whispers', time: '4:25', image: '/image/song-5.png' },
    { id: 6, title: 'Hidden Pathways', time: '3:15', image: '/image/song-6.png' },
    { id: 7, title: 'City Lights', time: '2:54', image: '/image/song-1.png' },
    { id: 8, title: 'Elizeu Dias', time: '4:32', image: '/image/song-7.png' },
    { id: 9, title: 'Obafemi Moyosade', time: '3:25', image: '/image/song-4.png' },
    { id: 10, title: 'Kamil Feczko', time: '2:05', image: '/image/song-6.png' },
  ];

  const handleSongToggle = (songId) => {
    setSelectedSongs(prev => ({
      ...prev,
      [songId]: !prev[songId]
    }));
  };

  const handleCancel = () => {
    setSelectedSongs({});
    onClose();
  };

  const handleConfirmPlaylist = () => {
    const selectedCount = Object.values(selectedSongs).filter(Boolean).length;
    alert(`Added ${selectedCount} songs to playlist!`);
    setSelectedSongs({});
    onClose();
  };

  if (!isOpen) return null;

  return (
    <div className="fixed inset-0 bg-black/80 flex items-center justify-center z-50 p-4">
      <div className="bg-[#312B36] rounded-lg w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        {/* Header */}
        <div className="flex items-center justify-between p-6 border-b border-[#896E9C]">
          <h2 className="text-xl font-medium text-[#F9FAFB]">My Song</h2>
          <button 
            onClick={onClose}
            className="p-2 hover:bg-[#1D1B25] rounded-lg transition-colors"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              className="h-5 w-5 text-[#F9FAFB]"
              viewBox="0 0 20 20"
              fill="currentColor"
            >
              <path
                fillRule="evenodd"
                d="M4.293 4.293a1 1 0 011.414 0L10 8.586l4.293-4.293a1 1 0 111.414 1.414L11.414 10l4.293 4.293a1 1 0 01-1.414 1.414L10 11.414l-4.293 4.293a1 1 0 01-1.414-1.414L8.586 10 4.293 5.707a1 1 0 010-1.414z"
                clipRule="evenodd"
              />
            </svg>
          </button>
        </div>

        {/* Content */}
        <div className="p-6">
          {/* Table */}
          <div className="overflow-x-auto rounded-md border border-[#896E9C] mb-6">
            <table className="min-w-full divide-y divide-[#896E9C]">
              <thead>
                <tr>
                  <th className="px-6 py-3 text-left text-xs font-medium text-[#F9FAFB] uppercase tracking-wider">
                    NO
                  </th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-[#F9FAFB] uppercase tracking-wider">
                    Song
                  </th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-[#F9FAFB] uppercase tracking-wider">
                    Time
                  </th>
                  <th className="px-6 py-3 text-left text-xs font-medium text-[#F9FAFB] uppercase tracking-wider">
                    Action
                  </th>
                </tr>
              </thead>
              <tbody className="divide-y divide-[#896E9C]">
                {songs.map((song, index) => (
                  <tr key={song.id}>
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-[#F9FAFB]">
                      {index + 1}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        <div className="flex-shrink-0 h-10 w-10">
                          <img
                            className="h-10 w-10 rounded-lg object-cover"
                            src={song.image}
                            alt={song.title}
                          />
                        </div>
                        <div className="ml-4">
                          <div className="text-sm font-medium text-[#F9FAFB]">
                            {song.title}
                          </div>
                        </div>
                      </div>
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-[#F9FAFB]">
                      {song.time}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap">
                      <button
                        onClick={() => handleSongToggle(song.id)}
                        className="p-2"
                      >
                        <div className={`w-6 h-6 rounded border-2 flex items-center justify-center ${
                          selectedSongs[song.id] 
                            ? 'bg-gradient-to-b from-[#FF7DD0] to-[#F7009E] border-[#FF7DD0]' 
                            : 'border-[#896E9C] hover:border-[#A38BB4]'
                        } transition-colors`}>
                          {selectedSongs[song.id] && (
                            <svg
                              xmlns="http://www.w3.org/2000/svg"
                              className="h-4 w-4 text-white"
                              viewBox="0 0 20 20"
                              fill="currentColor"
                            >
                              <path
                                fillRule="evenodd"
                                d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z"
                                clipRule="evenodd"
                              />
                            </svg>
                          )}
                        </div>
                      </button>
                    </td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>

          {/* Action Buttons */}
          <div className="flex justify-start space-x-4">
            <button
              onClick={handleCancel}
              className="px-6 py-3 bg-[#F7009E33] text-[#F9FAFB] rounded-md hover:bg-[#2A374B] transition-colors"
            >
              Cancel
            </button>
            <button
              onClick={handleConfirmPlaylist}
              className="px-6 py-3 bg-gradient-to-b from-[#FF7DD0] to-[#F7009E] hover:from-[#FF6BC9] hover:to-[#E6008F] text-white rounded-md transition-all duration-200"
            >
              Confirm Playlist
            </button>
          </div>
        </div>
      </div>
    </div>
  );
};

// Main Playlist Management Component
const PlaylistManagement = () => {
  const [playlists, setPlaylists] = useState([
    {
      id: 1,
      name: '2Am Vibes',
      visibility: 'Public',
      tracks: 40,
      updatedDate: '2025-05-23',
        avatarImage: '/image/song-1.png'
    },
    {
      id: 2,
      name: 'Ghetto Love',
      visibility: 'Public',
      tracks: 35,
      updatedDate: '2025-04-20',
        avatarImage: '/image/song-2.png'
    },
    {
      id: 3,
      name: 'Midnight Dreams',
      visibility: 'Public',
      tracks: 50,
      updatedDate: '2025-03-15',
        avatarImage: '/image/song-3.png'
    },
    {
      id: 4,
      name: 'Echoes of the Past',
      visibility: 'Public',
      tracks: 15,
      updatedDate: '2025-05-10',
        avatarImage: '/image/song-6.png'
    },
    {
      id: 5,
      name: 'Silent Whispers',
      visibility: 'Public',
      tracks: 10,
      updatedDate: '2025-01-05',
        avatarImage: '/image/song-2.png'
    },
    {
      id: 6,
      name: 'Rebeggeton',
      visibility: 'Public',
      tracks: 25,
      updatedDate: '2024-11-05',
      avatarImage: '/image/song-5.png'
    },
    {
      id: 7,
      name: 'amor of 90 by Bryan',
      visibility: 'Public',
      tracks: 35,
      updatedDate: '2024-10-03',
        avatarImage: '/image/song-4.png'
    },
    {
      id: 8,
      name: 'Four Strong Winds',
      visibility: 'Public',
      tracks: 14,
      updatedDate: '2024-06-05',
        avatarImage: '/image/song-1.png'
    },
    {
      id: 9,
      name: 'Tom Sawyer',
      visibility: 'Public',
      tracks: 18,
      updatedDate: '2024-03-25',
        avatarImage: '/image/song-2.png'
    },
    {
      id: 10,
      name: 'Grabbett',
      visibility: 'Public',
      tracks: 22,
      updatedDate: '2024-01-20',
      avatarImage: '/image/song-3.png'
    },
  ]);

  const [currentPage, setCurrentPage] = useState(1);
  const [isCreateModalOpen, setIsCreateModalOpen] = useState(false);
  const [isAddSongsModalOpen, setIsAddSongsModalOpen] = useState(false);
  const itemsPerPage = 10;
  const totalPages = Math.ceil(playlists.length / itemsPerPage);

  const paginatedPlaylists = playlists.slice(
    (currentPage - 1) * itemsPerPage,
    currentPage * itemsPerPage
  );

  const handlePageChange = (pageNumber) => {
    setCurrentPage(pageNumber);
  };

  const handleView = (id) => {
    alert(`Viewing playlist with ID: ${id}`);
  };

  const handleEdit = (id) => {
    alert(`Editing playlist with ID: ${id}`);
  };

  const handleDelete = (id) => {
    if (window.confirm(`Are you sure you want to delete playlist with ID: ${id}?`)) {
      setPlaylists(playlists.filter((playlist) => playlist.id !== id));
    }
  };

  const handleCreatePlaylist = () => {
    setIsCreateModalOpen(true);
  };

  const handleCloseCreateModal = () => {
    setIsCreateModalOpen(false);
  };

  const handlePlaylistCreated = (newPlaylist) => {
    setPlaylists(prevPlaylists => [newPlaylist, ...prevPlaylists]);
    // Open Add Songs modal after creating playlist
    setIsAddSongsModalOpen(true);
  };

  const handleCloseAddSongsModal = () => {
    setIsAddSongsModalOpen(false);
  };

  return (
    <div className="min-h-screen p-8 text-[#F9FAFB]">
      {/* Header */}
      <div className="flex justify-between items-center mb-8">
        <h1 className="text-2xl font-semibold">Playlist Management</h1>
        <button 
          onClick={handleCreatePlaylist}
          className="flex items-center px-4 py-2 bg-gradient-to-b from-[#FF7DD0] to-[#F7009E] hover:from-[#FF6BC9] hover:to-[#E6008F] cursor-pointer text-white rounded transition-colors"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            className="h-5 w-5 mr-2"
            viewBox="0 0 20 20"
            fill="currentColor"
          >
            <path
              fillRule="evenodd"
              d="M10 5a1 1 0 011 1v3h3a1 1 0 110 2h-3v3a1 1 0 11-2 0v-3H6a1 1 0 110-2h3V6a1 1 0 011-1z"
              clipRule="evenodd"
            />
          </svg>
          Create New Playlist
        </button>
      </div>

      {/* Main Content Area */}
      <div className="bg-[#312B36] rounded-lg p-6">
        {/* Search and Filter */}
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-xl font-semibold">Playlist</h2>
          <div className="flex space-x-4">
            <div className="relative">
              <input
                type="text"
                placeholder="Search"
                className="pl-10 pr-4 py-2 bg-[#1D1B25] text-white rounded-md border border-[#896E9C] focus:outline-none focus:border-[#A38BB4]"
              />
              <svg
                xmlns="http://www.w3.org/2000/svg"
                className="h-5 w-5 absolute left-3 top-1/2 transform -translate-y-1/2 text-gray-400"
                viewBox="0 0 20 20"
                fill="currentColor"
              >
                <path
                  fillRule="evenodd"
                  d="M8 4a4 4 0 100 8 4 4 0 000-8zM2 8a6 6 0 1110.89 3.476l4.817 4.817a1 1 0 01-1.414 1.414l-4.816-4.816A6 6 0 012 8z"
                  clipRule="evenodd"
                />
              </svg>
            </div>
            <button className="flex items-center px-4 py-2 bg-[#1D1B25] rounded-md border border-[#896E9C] hover:bg-[#2A374B] transition-colors">
              <svg
                xmlns="http://www.w3.org/2000/svg"
                className="h-5 w-5 mr-2"
                viewBox="0 0 20 20"
                fill="currentColor"
              >
                <path
                  fillRule="evenodd"
                  d="M3 3a1 1 0 011-1h12a1 1 0 011 1v3a1 1 0 01-.293.707L12 11.414V15a1 1 0 01-1.447.894l-2-1A1 1 0 017 14v-2.586L3.293 6.707A1 1 0 013 6V3z"
                  clipRule="evenodd"
                />
              </svg>
              Filter
            </button>
          </div>
        </div>

        {/* Table */}
        <div className="overflow-x-auto rounded-md border border-[#896E9C]">
          <table className="min-w-full divide-y divide-[#896E9C]">
            <thead>
              <tr>
                <th className="px-6 py-3 text-left text-xs font-medium text-[#F9FAFB] uppercase tracking-wider">
                  NO
                </th>
                <th className="px-6 py-3 text-left text-xs font-medium text-[#F9FAFB] uppercase tracking-wider">
                  Name
                </th>
                <th className="px-6 py-3 text-left text-xs font-medium text-[#F9FAFB] uppercase tracking-wider">
                  Visibility
                </th>
                <th className="px-6 py-3 text-left text-xs font-medium text-[#F9FAFB] uppercase tracking-wider">
                  Tracks
                </th>
                <th className="px-6 py-3 text-left text-xs font-medium text-[#F9FAFB] uppercase tracking-wider">
                  Updated Date
                </th>
                <th className="px-6 py-3 text-right text-xs font-medium text-[#F9FAFB] uppercase tracking-wider">
                  Actions
                </th>
              </tr>
            </thead>
            <tbody className=" divide-[#896E9C]">
              {paginatedPlaylists.map((playlist) => (
                <tr key={playlist.id}>
                  <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-[#F9FAFB]">
                    {playlist.id}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex items-center">
                      <div className="flex-shrink-0 h-10 w-10">
                        <img
                          className="h-10 w-10 rounded-lg object-cover"
                          src={playlist.avatarImage}
                          alt={playlist.name}
                        />
                      </div>
                      <div className="ml-4">
                        <div className="text-sm font-medium text-[#F9FAFB]">
                          {playlist.name}
                        </div>
                      </div>
                    </div>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className="px-6 py-1 inline-flex text-xs leading-5 font-semibold rounded bg-[#2BA84933] text-[#53C31B]">
                      {playlist.visibility}
                    </span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-[#F9FAFB]">
                    {playlist.tracks}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-[#F9FAFB]">
                    {playlist.updatedDate}
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <button
                      onClick={() => handleView(playlist.id)}
                      className="text-[#245FE7] bg-[#2196F333] hover:text-blue-600 mr-3 px-6 py-1 rounded-md transition-colors"
                    >
                      View
                    </button>
                    <button
                      onClick={() => handleEdit(playlist.id)}
                      className="text-yellow-400 bg-[#45381F] hover:text-yellow-600 mr-3 px-8 py-1 rounded-md transition-colors"
                    >
                      Edit
                    </button>
                    <button
                      onClick={() => handleDelete(playlist.id)}
                      className="text-[#FE4D4F] bg-[#551214] hover:text-red-600 px-6 py-1 rounded-md transition-colors"
                    >
                      Delete
                    </button>
                  </td>
                </tr>
              ))}
            </tbody>
          </table>
        </div>

        {/* Pagination */}
        <div className="flex justify-end items-center mt-6 space-x-2">
          <button
            onClick={() => handlePageChange(currentPage - 1)}
            disabled={currentPage === 1}
            className="px-3 py-1 rounded-md bg-[#896E9C] disabled:opacity-50 hover:bg-[#A38BB4] transition-colors"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              className="h-5 w-5"
              viewBox="0 0 20 20"
              fill="currentColor"
            >
              <path
                fillRule="evenodd"
                d="M12.707 5.293a1 1 0 010 1.414L9.414 10l3.293 3.293a1 1 0 01-1.414 1.414l-4-4a1 1 0 010-1.414l4-4a1 1 0 011.414 0z"
                clipRule="evenodd"
              />
            </svg>
          </button>
          
          {[...Array(Math.min(3, totalPages))].map((_, index) => (
            <button
              key={index + 1}
              onClick={() => handlePageChange(index + 1)}
              className={`px-3 py-1 rounded-md ${
                currentPage === index + 1
                  ? 'bg-[#896E9C]'
                  : 'bg-[#1E293B] border border-[#896E9C]'
              } hover:bg-[#A38BB4] transition-colors`}
            >
              {index + 1}
            </button>
          ))}
          <span className="px-3 py-1 text-[#F9FAFB]">...</span>
          <button
            onClick={() => handlePageChange(totalPages)}
            className={`px-3 py-1 rounded-md ${
              currentPage === totalPages
                ? 'bg-[#896E9C]'
                : 'bg-[#1E293B] border border-[#896E9C]'
            } hover:bg-[#A38BB4] transition-colors`}
          >
            {totalPages}
          </button>
          <button
            onClick={() => handlePageChange(currentPage + 1)}
            disabled={currentPage === totalPages}
            className="px-3 py-1 rounded-md bg-[#896E9C] disabled:opacity-50 hover:bg-[#A38BB4] transition-colors"
          >
            <svg
              xmlns="http://www.w3.org/2000/svg"
              className="h-5 w-5"
              viewBox="0 0 20 20"
              fill="currentColor"
            >
              <path
                fillRule="evenodd"
                d="M7.293 14.707a1 1 0 010-1.414L10.586 10 7.293 6.707a1 1 0 011.414-1.414l4 4a1 1 0 010 1.414l-4 4a1 1 0 01-1.414 0z"
                clipRule="evenodd"
              />
            </svg>
          </button>
        </div>
      </div>

      {/* Create New Playlist Modal */}
      <CreateNewPlaylistModal 
        isOpen={isCreateModalOpen}
        onClose={handleCloseCreateModal}
        onPlaylistCreated={handlePlaylistCreated}
      />

      {/* Add Songs Modal */}
      <AddSongsModal 
        isOpen={isAddSongsModalOpen}
        onClose={handleCloseAddSongsModal}
      />
    </div>
  );
};

export default PlaylistManagement;